...
import {ToHtlcSwap, ToHtlcSwapState} from "./ToHtlcSwap";
import {HTLC, HTLCStatus, IGeneralHTLCWallet} from "../../../wallets/IGeneralHTLCWallet";

export type ToHtlcConfig = ToBtcBaseConfig & {
    feeRateMultiplier: bigint,

    minSendCltv: bigint,

    htlcSendBitcoinBlockTimeSafetyFactorPPM?: bigint,

    txCheckInterval: number
};

export type ToHtlcRequestType = {
    pr: string,
    maxFee: bigint,
 ...
 */
export abstract class ToHtlc extends ToBtcBaseSwapHandler<ToHtlcSwap, ToHtlcSwapState> {
    readonly swapType = ChainSwapType.HTLC;
    readonly inflightSwapStates = new Set([ToHtlcSwapState.COMMITED, ToHtlcSwapState.PAID]);

    activeSubscriptions: Set<string> = new Set<string>();

    readonly config: ToHtlcConfig & {minTsSendCltv: bigint};

    readonly htlcWallet: IGeneralHTLCWallet;
    readonly LightningAssertions: LightningAssertions;

    constructor(
        storageDirectory: IIntermediaryStorage<ToHtlcSwap>,
        path: string,
        chainData: MultichainData,
        htlcWallet: IGeneralHTLCWallet,
        ...
        this.config.htlcSendBitcoinBlockTimeSafetyFactorPPM = this.config.htlcSendBitcoinBlockTimeSafetyFactorPPM ?? (this.config.safetyFactor * 1_000_000n);
        if(this.config.htlcSendBitcoinBlockTimeSafetyFactorPPM <= 1_100_000n) {
            throw new Error("HTLC send block safety factor set below 1.1, this is insecure!");
        }
    }

    protected async processPastSwap(swap: ToHtlcSwap): Promise<void> {
        const {swapContract, signer} = this.getChain(swap.chainIdentifier);

        if (swap.state === ToHtlcSwapState.SAVED) {
            ...
            if(isSignatureExpired) {
                ...
                if(!isCommitted) {
                    ...
                    await this.removeSwapData(swap, ToHtlcSwapState.CANCELED);
                    return;
                } else {
                    this.swapLogger.info(swap, "processPastSwap(state=SAVED): swap committed (detected from processPastSwap), invoice: "+swap.pr);
                    await swap.setState(ToHtlcSwapState.COMMITED);
                    await this.saveSwapData(swap);
                }
            }
        }

        if (swap.state === ToHtlcSwapState.COMMITED || swap.state === ToHtlcSwapState.PAID) {
            //Process swaps in commited & paid state
            await this.processInitialized(swap);
        }

        if (swap.state === ToHtlcSwapState.NON_PAYABLE) {
        ...
        const queriedData = await this.storageManager.query([
            {
                key: "state",
                value: [
                    ToHtlcSwapState.SAVED,
                    ToHtlcSwapState.COMMITED,
                    ToHtlcSwapState.PAID,
                    ToHtlcSwapState.NON_PAYABLE
                ]
            }
        ]);
     ...
     */
    private async tryClaimSwap(swap: ToHtlcSwap): Promise<boolean> {
        ...
        if(!isCommited) {
            ...
            if(status?.type===SwapCommitStateType.PAID) {
                ...
                await this.removeSwapData(swap, ToHtlcSwapState.CLAIMED);
                return true;
                ...
                await this.removeSwapData(swap, ToHtlcSwapState.REFUNDED);
            }
        ...
        try {
            ...
            this.swapLogger.info(swap, "tryClaimSwap(): swap claimed successfully, secret: "+swap.secret+" htlcId: "+swap.htlcId+" recipient: "+swap.recipient);
            if(swap.metadata!=null) swap.metadata.times.txClaimed = Date.now();
            unlock();
            return true;
        } catch (e) {
            this.swapLogger.error(swap, "tryClaimSwap(): error occurred claiming swap, secret: "+swap.secret+" invoice: "+swap.htlcId+" recipient: "+swap.recipient, e);
            return false;
        }
    }

    /**
     * Process the result of an offered HTLC
     *
     * @param swap
     * @param htlc
     */
    private async processHtlcResult(swap: ToHtlcSwap, htlc: HTLC) {
        switch(htlc.status) {
            case HTLCStatus.CONFIRMATION_PENDING:
            case HTLCStatus.CONFIRMED:
                return;

            case HTLCStatus.REFUNDABLE:
            case HTLCStatus.EXPIRED:
                this.swapLogger.info(swap, "processPaymentResult(): invoice payment failed, cancelling swap, htlcId: "+swap.htlcId+" recipient: "+swap.recipient);
                await swap.setState(ToHtlcSwapState.NON_PAYABLE);
                await this.saveSwapData(swap);
                return;

            case HTLCStatus.SETTLED:
                swap.secret = htlc.preimage;
                this.swapLogger.info(swap, "processPaymentResult(): dst htlc settled, secret: "+swap.secret+" htlcId: "+swap.htlcId+" recipient: "+swap.recipient);
                await swap.setState(ToHtlcSwapState.PAID);
                await this.saveSwapData(swap);

                const success = await this.tryClaimSwap(swap);
                if(success) this.swapLogger.info(swap, "processPaymentResult(): swap claimed successfully, htlcId: "+swap.htlcId+" recipient: "+swap.recipient);
                return;
    ...
    private async offerHtlc(swap: ToHtlcSwap): Promise<void> {
